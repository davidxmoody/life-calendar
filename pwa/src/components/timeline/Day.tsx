...
export default memo(function Day(props: Props) {
  ...
  const entry = useEntry(isExpanded ? props.date : null)
  ...
  return (
    <div className="max-w-[800px] md:px-2 pb-4 md:pb-2">
      <div className="relative md:rounded-md" style={{contain: "paint"}}>
        <DayHeader date={props.date} onClick={onToggle} />

        <div
          className={`
            md:border-l md:border-r md:border-b md:rounded-b-md border-gray-600
            overflow-hidden ${isExpanded ? "" : "cursor-pointer"}
          `}
...
function DayHeader(props: {date: string; onClick: () => void}) {
  return (
    <div className="sticky top-0 z-40 md:pt-2 bg-[#1a202c]">
      <div
...
function Full(props: {entry: Entry}) {
  return (
    <div className="mx-4 md:mx-8 my-4 md:my-6">
      <EntryDateProvider value={props.entry.date}>
